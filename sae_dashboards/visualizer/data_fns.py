...
from josephs_sae_dashboards.visualizer.html_fns import (
    CSS,
...
from josephs_sae_dashboards.visualizer.model_fns import AutoEncoder
from josephs_sae_dashboards.visualizer.utils_fns import (
    TopK,
...
def get_feature_data(
    ...
    if ("resid_pre" in hook_point or
        "attn_out" in hook_point or
        "resid_mid" in hook_point or
        "mlp_out" in hook_point or
        "resid_post" in hook_point):
    ...
    def hook_fn_resid(
        resid_acts: Float[Tensor, "batch seq d_model"], hook: HookPoint  # noqa
    ):
        """
        This hook function stores the residual activations activations at the specified hook point.
        """
        feat_acts = F.relu((resid_acts - encoder.b_dec) @ feature_act_dir + feature_bias)
        all_feat_acts.append(feat_acts)

        # Update the CorrCoef object between feature activation & neurons
        corrcoef_neurons.update(
            einops.rearrange(feat_acts, "batch seq feats -> feats (batch seq)"),
            einops.rearrange(resid_acts, "batch seq d_model -> d_model (batch seq)"),
        )
