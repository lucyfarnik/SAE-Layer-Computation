...
import wandb
from josephs_sae_dashboards.visualizer.data_fns import get_feature_data
...
class DashboardRunner:
    def __init__(
        ...
        wandb_sparsity_version: int = None,
        ...
        if wandb_artifact_path is not None:
            ...
            if not os.path.exists(artifact_dir):
                ...
                #! FIXME this is kinda cursed, we're creating a "run" just to
                #! download the artifact, at some point I should just refactor
                #! this to init() w the correct params and then skip the init()
                #! in the run method
                run = wandb.init()
                ...
                feature_sparsity_path = self.get_feature_sparsity_path(
                    wandb_artifact_path, wandb_sparsity_version,
                )
    ...
    def get_feature_sparsity_path(self, wandb_artifact_path, wandb_sparsity_version):
        prefix = wandb_artifact_path.split(":")[0]
        return f"{prefix}_log_feature_sparsity:v{wandb_sparsity_version}"
